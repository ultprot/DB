select 주차장.주소, 주차장.종류, 
		...
		convert(int,sqrt ( (0-주차장.x좌표)*(0-주차장.x좌표)+(0-주차장.y좌표)*(0-주차장.y좌표))) as distance, 주차장평점
...
count(case when 자리.현재_사용여부 = 0 and 자리.수용차종=2 then 1 end) as 대형빈자리수
...
where 주차장평점>=0 and 현재자리수>0 and 대형빈자리수>0 and 주차장.종류=1
ORDER BY distance;